...
export default function CarrinhoPage() {
  // Puxamos todas as informações do nosso cérebro global
  const { items, subtotal, atualizarQuantidade, removerItem } = useCart();

  const formatarPreco = (valor: number) =>
    valor.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });

  // Se não houver itens, mostramos a tela de vazio elegante
  if (items.length === 0) {
    return (
  ...
  // Se houver itens, mostramos a lista completa e o resumo do pedido
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-grow">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        {/* Coluna 1: Lista de Produtos Detalhada */}
              ...
              <div className="flex-1 text-center sm:text-left w-full">
                ...
                <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => removerItem(item.id)} className="text-red-500 hover:bg-red-50 p-2 rounded-full transition-colors" title="Remover item">
        ...
        {/* Coluna 2: Resumo do Pedido (Bridge para a Fase 5) */}
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-brand/10 h-fit sticky top-28">
          ...
          {/* O Botão da Fase 5: Integração WhatsApp */}
          <button className="w-full bg-[#25D366] text-white py-4 rounded-full font-bold text-lg shadow-lg hover:bg-[#1ebe5d] transition-colors flex justify-center items-center gap-2">
